						...
						<tr>
							...
							@if ( count($savedCategory) )
								@foreach ($savedCategory as $game_category)
									@if($game_category == 1)
										<th>@lang('app.hot_order')</th>
									@endif
									@if($game_category == 2)
										<th>@lang('app.new_order')</th>
									@endif
								@endforeach
							@endif
							<!-- --- -->
...
@section('scripts')
	<script>
		...
		$('.order-update').click(function () {
			var gameid = $(this).prev().attr('data-gameid');
			var order = $(this).prev().val();
			var ordertype="original";
			$.ajax({
				url: '/backend/game/orderupdate',
				type: 'POST',
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
				},
				data: {gameid: gameid, order: order, ordertype: ordertype},
				success:(data)=>{
					if (data.error) {
						alert(data.msg);
						return;
		...
		$('.hot-order-update').click(function () {
			var gameid = $(this).prev().attr('data-gameid');
			var order = $(this).prev().val();
			var ordertype="hot";
			$.ajax({
		...
		$('.new-order-update').click(function () {
			var gameid = $(this).prev().attr('data-gameid');
			var order = $(this).prev().val();
			var ordertype="new";
			$.ajax({
