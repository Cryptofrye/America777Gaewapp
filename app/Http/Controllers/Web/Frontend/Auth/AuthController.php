        ...
        public function phone_verify(\Illuminate\Http\Request $request) {
            ...
            if($existing_phone_check == 0) {
                return response(json_encode([
                    'type' => 'exist_error',
                    'message' => "Player to the phone number does not exit. You have to sign up on canada777.com",
                    'url' => route('frontend.game.list.category', ['category1'=>'hot', 'freespin_signup' => true])
            ...
            if($existing_promotion_check > 0){
                return response(json_encode([
                    'type' => 'exist_error',
                    'message' => "The phone number has already taken",
                    'url' => route('frontend.game.list')
                ]));
            }
