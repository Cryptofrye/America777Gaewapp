...
/*            $checked = new \VanguardLTE\Lib\LicenseDK();
        ...
        public function edit(\Illuminate\Http\Request $request, \VanguardLTE\Repositories\Activity\ActivityRepository $activitiesRepo, \VanguardLTE\User $user)
        {
            ...
            $transactions = \VanguardLTE\Transaction::where('user_id', $user->id)->orderBy('created_at', 'DESC')->get();
            $bets = \VanguardLTE\StatGame::where('user_id', $user->id)->orderBy('id','DESC')->get();
            $bonus = \VanguardLTE\BonusLog::where('user_id', $user->id)->get();
            ...
            $numbers = \DB::select("SELECT `game`, SUM(number) AS summ
                                    ...
                                    WHERE `user_id` =:userID AND `number` != \"\" AND `created_at` LIKE :mydate
                                    GROUP BY game
            ...
            return view('backend.user.edit', compact('edit', 'user', 'verify', 'roles', 'statuses', 'shops', 'countries', 'currencies', 'free_shops', 'userActivities', 'hasActivities', 'langs', 'max_wins', 'max_bets', 'numbers', 'transactions', 'bets', 'bonus', 'multiAccountsCount', 'multiAccounts'));
        }
        ...
        public function updateVerify(\VanguardLTE\User $user, \Illuminate\Http\Request $request)
        {
            if( \Illuminate\Support\Facades\Auth::user()->role_id < $user->role_id ) 
            {
                return redirect()->route('backend.user.list');
            }
            $curVerify = \VanguardLTE\Verify::where('user_id', $user->id)->first();
            if ($curVerify != null){
                $curVerify->verified = $request->verified;
                $curVerify->save();
            }
            return redirect()->back()->withSuccess(trans('app.user_updated'));
        }
