...
/*            $checked = new \VanguardLTE\Lib\LicenseDK();
        ...
        public function orderupdate(\Illuminate\Http\Request $request)
        {
            ...
            if($order_type == "original"){
                \VanguardLTE\Game::where(['original_id' => $gameid])->update(['order' => $order]);
            }
            if($order_type == "hot"){
                \VanguardLTE\Game::where(['original_id' => $gameid])->update(['hot_order' => $order]);
            }
            if($order_type == "new"){
                \VanguardLTE\Game::where(['original_id' => $gameid])->update(['new_order' => $order]);
            }
            /* --- */
            return response()->json(['error' => false, 'msg' => trans('app.order_updated_successfully')], 200);
        }
