<section id="category-section">
    ...
    <div class="mobile-top-category d-block d-md-none">
        <div class="mobile-top-category-list">
            @if( settings('use_all_categories') )
                <a href="{{ route('frontend.game.list.category', 'all') }}" class="ml-1 mr-1 pr-1 @if($currentSliderNum != -1 && $currentSliderNum == 'all') active @endif">@lang('app.all')</a>
            @endif
            @if ($categories)
                @foreach($categories as $category)
                    @if($category->position < 7)
                        <a href="{{ route('frontend.game.list.category', $category->href) }}" class="ml-1 mr-1 p-1 @if($currentSliderNum != -1 && $category->href == $currentSliderNum) active @endif">{{ $category->title }}</a>
                    @endif
                @endforeach
            @endif
        </div>
                    ...
                    @foreach($categories as $category)
                        <li>
                            ...
                            <span class="providers__dropdown-icon">
                                <img class="providers__icon-img" src="{{asset('frontend/Page/image/icon')}}/{{strtolower($category->href).'.svg'}}">
                            </span>
