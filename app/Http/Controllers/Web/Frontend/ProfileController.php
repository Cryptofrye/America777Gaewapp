...
namespace VanguardLTE\Http\Controllers\Web\Frontend
{
    ...
    class ProfileController extends \VanguardLTE\Http\Controllers\Controller
    {
                    ...
                    if ($wager_played > 0) {
                        ...
                        foreach ($bonus_logs as $bonus_log) {
                            ...
                            if ($wager_played > $wager_remaining) {
                                ...
                                $this->theUser->incrument('bonus', -1 * min($bonus_log->bonus, $this->theUser->bonus));
                            }
                            else {
                        ...
                        foreach ($welcomepackage_logs as $welcomepackage_log) {
                            ...
                            if ($wager_played > $wager_remaining) {
                                ...
                                $wager_played = $wager_played - $wager_remaining;
                                $this->theUser->incrument('bonus', -1 * min($wager / 70, $this->theUser->bonus));
                            }
                            else {
        ...
        public function bonus_history(\Illuminate\Http\Request $request)
        {
            $tab = "history";
            $currencys =  \VanguardLTE\Currency::orderBy('ranking','ASC')->get();
            ...
            $html = view('frontend.Default.user.bonus_history')->with(compact('tab','currencys','welcomepackage_history', 'bonus_history'))->render();
            return response()->json(['success' => true, 'html' => $html]);
        }
        public function withdraw_history(\Illuminate\Http\Request $request)
        {
            $tab = "history";
            $currencys =  \VanguardLTE\Currency::orderBy('ranking','ASC')->get();
            $welcomepackage_history = \VanguardLTE\WelcomePackageLog::leftJoin('games', function ($join)
        ...
        public function verify(\Illuminate\Http\Request $request)
        {
            ...
            $html = view('frontend.Default.user.verify')->with(compact('idVerified','addressVerified'))->render();
            return response()->json(['success' => true, 'html' => $html]);
        }
        ...
        public function deposit(\Illuminate\Http\Request $request)
        {
            ...
            $currency =  \VanguardLTE\Currency::where('id', \Auth::user()->currency)->first();
            $currencys =  \VanguardLTE\Currency::orderBy('ranking','ASC')->get();
            $html = view('frontend.Default.user.deposit')->with(compact('tab','currency','currencys'))->render();
            return response()->json(['success' => true, 'html' => $html]);
        }
